<template>
  <button
    :class="buttonStyles"
    class="inline-flex items-center justify-center gap-2 rounded-md text-sm font-medium ring-offset-white transition-colors duration-300 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-yellow-dark focus-visible:ring-offset-1 disabled:pointer-events-none [&_.icon]:pointer-events-none [&_.icon]:size-5 [&_.icon]:shrink-0 dark:focus-visible:ring-yellow-dark active:scale-[.965] disabled:cursor-not-allowed select-none relative disabled:opacity-60"
...
import { ButtonEmits, ButtonProps } from "@/types/components";
...
const variants: Record<TButtonVariants, TClassName> = {
  primary: "bg-yellow text-dark hover:bg-yellow-dark",
  danger: "bg-red/10 text-red hover:bg-red/15",
  gray: "bg-gray-100 hover:bg-gray-150 text-gray-900 disabled:text-gray-1",
  red: "bg-red text-white hover:bg-[#D62B2D]",
  white: "bg-transparent text-dark-black hover:bg-gray-100",
};

const sizes: Record<TButtonSizes, TClassName> = {
  sm: "py-2.5 px-4 text-xs font-medium rounded-md",
  md: "py-3 px-5 font-medium text-base rounded-lg",
};
...
const loaderPathClass = computed(() => {
  ...
  } else if (["outline-dark", "primary"].includes(props.variant)) {
    return "fill-dark";
  }
  return "fill-white";
});
