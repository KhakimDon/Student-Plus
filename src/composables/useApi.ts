...
import { generateFingerprint } from "@/utils/functions/common";
...
export const useApi = (apiUrl?: string) => {
  ...
  function $service(options?: CreateAxiosDefaults): AxiosInstance {
    const getHeaders = () => ({
      ...
      Authorization: authStore.getTokens.access
        ? "Bearer " + authStore.getTokens.access
        : undefined,
    ...
    instance.interceptors.response.use(
      ...
      async function (error: AxiosError) {
        if (
          ...
          error.config.url !== "/auth/TokenRefresh/"
        ) {
          try {
